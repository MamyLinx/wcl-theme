...
*/
    define('BENDER_BLACK_THEME_VERSION', '320');
    if( (string)osc_get_preference('keyword_placeholder', 'bender_black')=="" ) {
        Params::setParam('keyword_placeholder', __('ie. PHP Programmer', 'bender_black') ) ;
    }
    ...
    if(!OC_ADMIN) {
        ...
        if(getPreference('rtl','bender_black')=='0') {
            osc_enqueue_style('bender_black-fine-uploader-css', osc_current_web_theme_url('css/ajax-uploader.css'));
        } else {
            osc_enqueue_style('bender_black-fine-uploader-css', osc_current_web_theme_url('css/ajax-uploader-rtl.css'));
        }
    }
...
*/
    ...
    if( !function_exists('bender_black_theme_install') ) {
        function bender_black_theme_install() {
            osc_set_preference('keyword_placeholder', Params::getParam('keyword_placeholder'), 'bender_black');
            osc_set_preference('version', BENDER_BLACK_THEME_VERSION, 'bender_black');
            osc_set_preference('footer_link', '1', 'bender_black');
            osc_set_preference('donation', '0', 'bender_black');
            osc_set_preference('defaultShowAs@all', 'list', 'bender_black');
            osc_set_preference('defaultShowAs@search', 'list');
            osc_set_preference('defaultLocationShowAs', 'dropdown', 'bender_black'); // dropdown / autocomplete
            osc_set_preference('rtl', '0', 'bender_black');
            osc_reset_preferences();
        }
    }
    // update options
    if( !function_exists('bender_black_theme_update') ) {
        function bender_black_theme_update($current_version) {
            if($current_version==0) {
                bender_black_theme_install();
            }
            osc_delete_preference('default_logo', 'bender_black');

            $logo_prefence = osc_get_preference('logo', 'bender_black');
            ...
            if( file_exists( $temp_name ) && !$logo_prefence) {
                ...
                osc_set_preference('logo', $logo_name, 'bender_black');
            }
            osc_set_preference('version', '301', 'bender_black');

            if($current_version<313 || $current_version=='3.0.1') {
                // add preferences
                osc_set_preference('defaultLocationShowAs', 'dropdown', 'bender_black');
                osc_set_preference('version', '313', 'bender_black');
            }
            osc_set_preference('version', '314', 'bender_black');
            if($current_version<320 ) {
                // add preferences
                osc_set_preference('rtl', '0', 'bender_black');
                osc_set_preference('version', '320', 'bender_black');
            }
            osc_set_preference('version', '320', 'bender_black');
            osc_reset_preferences();
        }
    ...
    if(!function_exists('check_install_bender_black_theme')) {
        function check_install_bender_black_theme() {
            ...
            if( $current_version=='' ) {
                bender_black_theme_update(0);
            } else if($current_version < BENDER_BLACK_THEME_VERSION){
                bender_black_theme_update($current_version);
            }
        }
    }
    ...
    if( !function_exists('logo_header') ) {
        function logo_header() {
             $logo = osc_get_preference('logo','bender_black');
    ...
    if( !function_exists('bender_black_default_direction') ){
        function bender_black_default_direction(){
            return getPreference('rtl','bender_black');
        }
    }
    if( !function_exists('bender_black_default_show_as') ){
        function bender_black_default_show_as(){
            return getPreference('defaultShowAs@all','bender_black');
        }
    }
    if( !function_exists('bender_black_default_location_show_as') ){
        function bender_black_default_location_show_as(){
            return osc_get_preference('defaultLocationShowAs','bender_black');
        }
    }
    if( !function_exists('bender_black_draw_categories_list') ) {
        function bender_black_draw_categories_list(){ ?>
        <?php if(!osc_is_home_page()){ echo '<div class="container">'; } ?>
        <?php
         //cell_3
        $total_categories   = osc_count_categories();
        $col1_max_cat       = ceil($total_categories/5);
        ...
        <ul class="r-list">
             <li>
                 <h1 class="h5">
                    ...
                    <span class="collapse resp-toogle"><i class="fa fa-plus"></i></span>
                    <?php } ?>
                    <?php if($_total_items > 0) { ?>
                    <a class="category <?php echo $_slug; ?>" href="<?php echo $_url; ?>"> 
	                    <small><?php echo $_total_items ; ?></small>
	                 </a>
                    <?php } else { ?>
                   ...
                   <ul>
                   		 <?php 
	                   		 ...
	                   		 while ( osc_has_subcategories() ) {
	                         	if($flag==4) { 
                         ?>
                         <div id="<?php echo $_slug; ?>" class="collapse">
                         <?php } ?>
                             <li>
	                             <?php if( osc_category_total_items() > 0 ) { ?>
	                                 <a class="category sub-category <?php echo osc_category_slug() ; ?>" href="<?php echo osc_search_category_url() ; ?>">
		                                 ...
		                                 <small><?php echo osc_category_total_items() ; ?></small>
	                                 </a>
	                             <?php } else { ?>
	                                 <a class="category sub-category <?php echo osc_category_slug() ; ?>" href="#">
		                                 <?php echo osc_category_name() ; ?>
		                                 <small><?php echo osc_category_total_items() ; ?></small>
	                                 </a>
	                             <?php } ?>
                         ...
                         ?>
                         <li>
                         	<a class="text-primary category sub-category collapsed" data-toggle="collapse" data-target="#<?php echo $_slug; ?>"><span class="plus">Plus...</span><span class="minus">...Moins</span></a></li>
                   </ul>
                 <?php } ?>
             </li>
        </ul>
    ...
    if( !function_exists('bender_black_draw_categories_menu') ) {
        function bender_black_draw_categories_menu(){ ?>
        <?php while ( osc_has_categories() ) {?>
        <div class="col mb-0 p-1">
             <div class="dropright">
                 <?php
                 $_slug      = osc_category_slug();
                 $_url       = osc_search_category_url();
                 ...
                 ?>
                 <?php if($_total_items > 0) { ?>
	                 <a class="btn btn-light category-button dropdown-toggle w-100" href="<?php echo $_url; ?>" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php echo $_name ; ?><small class="badge badge-info ml-1"><?php echo $_total_items ; ?></small>
						  </a>
	                 <?php } else { ?>
	                 <a class="btn btn-light category-button dropdown-toggle w-100" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php echo $_name ; ?><small class="badge badge-info ml-1"><?php echo $_total_items ; ?></small>
						  </a>
                 <?php } ?>
                 <?php if ( osc_count_subcategories() > 0 ) { ?>
                   <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                   		 <?php  while ( osc_has_subcategories() ) { ?>
	                             <?php if( osc_category_total_items() > 0 ) { ?>
	                                 <a class="dropdown-item" href="<?php echo osc_search_category_url() ; ?>">
		                                 <?php echo osc_category_name() ; ?>
		                                 <small class="badge badge-info"><?php echo osc_category_total_items() ; ?></small>
	                                 </a>
	                             <?php } else { ?>
    ...
    function theme_bender_black_actions_admin() {
        ...
        if( Params::getParam('file') == 'oc-content/themes/bender_black/admin/settings.php' ) {
            if( Params::getParam('donation') == 'successful' ) {
                osc_set_preference('donation', '1', 'bender_black');
                osc_reset_preferences();
            }
        }
        ...
        switch( Params::getParam('action_specific') ) {
            case('settings'):
                ...
                osc_set_preference('keyword_placeholder', Params::getParam('keyword_placeholder'), 'bender_black');
                osc_set_preference('footer_link', ($footerLink ? '1' : '0'), 'bender_black');
                osc_set_preference('defaultShowAs@all', Params::getParam('defaultShowAs@all'), 'bender_black');
                osc_set_preference('defaultShowAs@search', Params::getParam('defaultShowAs@all'));

                osc_set_preference('defaultLocationShowAs', Params::getParam('defaultLocationShowAs'), 'bender_black');
                ...
                osc_set_preference('rtl', (Params::getParam('rtl') ? '1' : '0'), 'bender_black');
            ...
            case('remove'):
                ...
                if(file_exists( $path ) ) {
                    @unlink( $path );
                    osc_delete_preference('logo','bender_black');
                    osc_reset_preferences();
...
//if(osc_is_home_page()){
 osc_add_hook('inside-main','bender_black_draw_categories_menu');
/*} else if( osc_is_static_page() || osc_is_contact_page() ){
    osc_add_hook('before-content','bender_black_draw_categories_menu');
}*/
