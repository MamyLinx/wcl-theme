...
<?php if( is_writable( osc_uploads_path()) ) { ?>
    <?php if($logo_prefence) { ?>
        ...
        <img border="0" alt="<?php echo osc_esc_html( osc_page_title() ); ?>" src="<?php echo bender_black_logo_url().'?'.filemtime(osc_uploads_path() . osc_get_preference('logo','bender_black'));?>" />
    ...
    <p><?php _e('The preferred size of the logo is 600x100.', 'bender_black'); ?></p>
    <?php if( $logo_prefence ) { ?>
    <div class="flashmessage flashmessage-inline flashmessage-warning"><p><?php _e('<strong>Note:</strong> Uploading another logo will overwrite the current logo.', 'bender_black'); ?></p></div>
    <?php } ?>
    <br/><br/>
    <form action="<?php echo osc_admin_render_theme_url('oc-content/themes/bender_black/admin/header.php'); ?>" method="post" enctype="multipart/form-data" class="nocsrf">
