<?php
    ...
    function custom_meta_title($data){
        return __('Change username', 'bender_black');;
    }
...
$(document).ready(function() {
    $('form#change-username').validate({
        ...
        messages: {
            s_username: {
                required: '<?php echo osc_esc_js(__("Username: this field is required", "bender_black")); ?>.'
            }
        },
            ...
            cInterval = setInterval(function(){
                $.getJSON(
                    ...
                    function(data){
                        clearInterval(cInterval);
                        if(data.exists==0) {
                            $("#available").text('<?php echo osc_esc_js(__("The username is available", "bender_black")); ?>');
                        } else {
                            $("#available").text('<?php echo osc_esc_js(__("The username is NOT available", "bender_black")); ?>');
                        }
                    }
                );
            }, 1000);
...
<div class="card col-md-6 mx-auto px-4">
    <div class="card-body">
    	<div class="card-title">
	    	<span class="h4"><?php _e('Change username', 'bender_black'); ?></span>
    	</div><hr>
        <ul id="error_list"></ul>
        <form action="<?php echo osc_base_url(true); ?>" method="post" id="change-username">
            ...
            <div class="control-group">
                <div class="controls">
                    <button type="submit" class="btn btn-primary mt-2"><?php _e("Update", 'bender_black');?></button>
                </div>
            </div>
        </form>
    </div>
